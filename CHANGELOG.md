...
- two new deterministic bundled demos covering command injection and server-side template injection;
- primary demonstrations of `pattern_break` and `state_dependency`, bringing committed transformation coverage to all five resistance patterns;
- a JSON challenge-adapter contract plus executable adapters for every bundled demo;
- adapter validation and E2E execution in the unified research-artifact verifier;
- a generated structural-generalization snapshot across bundled and external transformation reports;
- regression tests that prevent bundled deterministic demos from being counted as fresh-solver observations.

### Changed

- the documented artifact set now contains five bundled demos and two independently authored external transformations;
- structural generalization is reported separately from empirical solver evidence and human-subject claims.

### Fixed

- release-time Software Heritage preservation no longer relies solely on a `release: published` workflow event generated by the same `GITHUB_TOKEN`; new releases issue a direct best-effort archival request, while the independent archive workflow remains available for manual and maintenance-triggered preservation.

### v0.5.0 evidence boundary

v0.5.0 adds **no new paid model observations**. The empirical solver dataset remains the unchanged v0.3.0/v0.4.0 dataset: 50 raw attempts, 23 valid solver observations, and 27 infrastructure failures.

The new five-demo adapter set and seven-transformation matrix are deterministic/structural evidence. They do not establish population-level LLM generalization, empirically unchanged human difficulty, or cross-model replication.
