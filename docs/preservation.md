...
The maintainer's GitHub account and `aleff-github/NiceTryGPT` repository are connected to Zenodo. The v0.5.0 release metadata intentionally does not reuse the v0.2.0 DOI. Once Zenodo mints the v0.5.0 deposit, its version-specific DOI should be added to `CITATION.cff`, CodeMeta, the website, and citation guidance in one synchronization update.
