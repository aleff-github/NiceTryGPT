...
RUNTIME_ROOT = Path(tempfile.mkdtemp(prefix="nicetrygpt-traversal-after-"))
PUBLIC_DIR = RUNTIME_ROOT / "public"
EXPORTS_DIR = RUNTIME_ROOT / "exports"
EXPORT_NAME = f"report-{secrets.token_hex(4)}.txt"

PUBLIC_DIR.mkdir(parents=True)
EXPORTS_DIR.mkdir(parents=True)
(PUBLIC_DIR / "welcome.txt").write_text("Welcome to TinyFiles.\n", encoding="utf-8")
(EXPORTS_DIR / EXPORT_NAME).write_text(FLAG + "\n", encoding="utf-8")
    ...
    def do_GET(self):
        ...
        if parsed.path == "/":
            return self.send_text(
                ...
                "Recent export activity: /api/activity\n",
            )

        if parsed.path == "/api/activity":
            return self.send_text(
                200,
                json.dumps({"latest_export": f"exports/{EXPORT_NAME}"}) + "\n",
            )
